	...
	<div class="body-content" >
		...
		<div class="topbar-bottom">
			...
			<div class="left" style="width: 50%">
				...
				<h2>Order Date :  {{ $order['created_at']->toDayDateTimeString() }}</h2>
				<h2>Customer Name :  {{ $order->name }}</h2>
				<h2>Customer Number :  {{ $order->phone }}</h2>
			</div>
            <!-- left part end -->
		...
		<div class="topbar-table"  >
			<table style="width: 100%;" style="overflow: scroll; overflow-y:hidden;">
				...
				<tbody>
					<!-- item start -->
					@foreach( $order->order_item as $orders )
					<tr>
						<td>
							<img width="32px" src="{{ asset('images/menu/'. $orders['image']) }}" alt="">
						</td>
						<td>{{ $orders['item'] }} tk</td>
						<td>{{ $orders['price'] }} tk</td>
						<td>{{ $orders['qty']}} tk</td>
						<td>{{ $orders['total'] }} tk</td>

	...
	<div class="body-content" >
		...
		<div class="topbar-bottom">
			...
			<div class="left" style="width: 50%">
				@if( $invoice->status == 1 )
				<h2 class="badge-success">Order is now pending</h2>
				@elseif( $invoice->status == 4 )
				<h2 class="badge-danger">Order is cancelled</h2>
				@endif
        ...
        <!-- topbar table end -->
        @if( $invoice->status == 1 )
        <div class="topbar-table text-right"  >
            <a href="{{ route('confirm-order', $invoice->id) }}" class=" btn-primary">Confirmed Order</a>
            <a href="{{ route('cancelled-order', $invoice->id) }}" class=" btn-primary">Cancelled Order</a>
		</div>
		@endif
